...
export class ClashOfClansError extends Error {
    ...
    static #getErrorMessage(error) {
        ...
        switch (errno) {
            case HTTP_400_BAD_REQUEST: return 'Client provided incorrect parameters for the request.';
            case HTTP_403_FORBIDDEN: return 'Access denied, either because of missing/incorrect credentials or used API token does not grant access to the requested resource.';
            case HTTP_404_NOT_FOUND: return 'Resource was not found.';
            case HTTP_429_TOO_MANY_REQUESTS: return 'Request was throttled, because amount of requests was above the threshold defined for the used API token.';
            case HTTP_500_INTERNAL_SERVER_ERROR: return 'Unknown error happened when handling the request.';
            case HTTP_503_SERVICE_UNAVAILABLE: return 'Service is temprorarily unavailable because of maintenance.';
            case HTTP_504_GATEWAY_TIMEOUT: return 'The server did not get a response in time from the upstream server that it needed in order to complete the request.'
