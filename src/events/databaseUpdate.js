    ...
    try {
        ...
        for (const playerClan of playersClan) {
            ...
            try {
                ...
                if (error instanceof DatabaseError) {
                    if (error.code === SQLITE_CONSTRAINT_FOREIGNKEY) {  // player dont exists
                        await Database.runCommand(db, ` INSERT INTO PlayerData (tag, name, townHall, warPreference, lootCapitalT, addCapitalT, clanGamesT)
            ...
            if (playerDatabase.lootCapitalT !== lootCapitalT.value) { // lootCapitalT changed
                ...
                await Database.runCommand(db, `UPDATE PlayerClanData SET lootCapital = '${lootCapitalNew + lootCapitalOld.lootCapital}' WHERE clan = '${playerClan.clan.tag}' AND player = '${playerClan.tag}'`);
            ...
            if (playerDatabase.addCapitalT !== addCapitalT.value) { // addCapitalT changed
                ...
                await Database.runCommand(db, `UPDATE PlayerClanData SET addCapital = '${addCapitalNew + addCapitalOld.addCapital}' WHERE clan = '${playerClan.clan.tag}' AND player = '${playerClan.tag}'`);
            ...
            if (playerDatabase.clanGamesT !== clanGamesT.value) { // clanGamesT changed
                ...
                await Database.runCommand(db, `UPDATE PlayerClanData SET clanGames = '${clanGamesNew + clanGamesOld.clanGames}' WHERE clan = '${playerClan.clan.tag}' AND player = '${playerClan.tag}'`);
